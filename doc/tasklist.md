...
- [ ] Исследовать и выбрать подход к i18n в Yew (fluent-rs, yew-i18n, или custom решение)
- [ ] Разработать типобезопасную структуру для переводов
- [ ] Создать enum Language с поддерживаемыми языками (ru, en)
- [ ] Реализовать Context Provider для текущего языка
- [ ] Добавить LocalStorage для сохранения выбранного языка
- [ ] Реализовать автоопределение языка браузера (navigator.language)

#### 🔲 Инфраструктура переводов
- [ ] Создать структуру директорий `assets/i18n/{lang}/content.yaml`
- [ ] Разработать макрос или helper для типобезопасного доступа к переводам
- [ ] Реализовать fallback механизм (язык -> английский -> русский)
...
#### 🔲 UI компоненты
- [ ] Создать LanguageSwitcher компонент с выпадающим меню
- [ ] Добавить флаги стран для визуального представления языков
- [ ] Разместить переключатель в header и footer
...
#### 🔲 Переводы контента
- [ ] **Русский (ru)** - основной язык, уже существует
- [ ] **Английский (en)** - перевести весь контент и UI
  - [ ] Hero секция и CTA кнопки
  - [ ] Секция "Возможности" (Features)
  - [ ] Секция "Тарифы" (Pricing)
  - [ ] Секция "Безопасность" (Security)
  - [ ] FAQ (все 13 вопросов)
  - [ ] Примеры диалогов (4+ примера)
  - [ ] Footer с контактами
...
#### 🔲 SEO и meta-теги
- [ ] Добавить `<html lang="XX">` атрибут с текущим языком
